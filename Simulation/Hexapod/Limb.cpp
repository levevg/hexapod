...
void Limb::moveKnee(float normalized, uint16_t time)
{
	currentKneeNormalized = normalized;
	if (enabled) _knee->move(normalized, time);
}

void Limb::moveWrist(float normalized, uint16_t time)
{
	currentWristNormalized = normalized;
	if (enabled) _wrist->move(normalized, time);
}

void Limb::moveShoulder(float normalized, uint16_t time)
{
	currentShoulderNormalized = normalized;
	if (enabled) _shoulder->move(normalized, time);
}
...
void Limb::moveToPWMWidth(uint16_t kneeWidth, uint16_t wristWidth, uint16_t shoulderWidth, const uint16_t time)
{
	...
	if (enabled) servosPort.send(b, 12);
}

void Limb::updateCurrentPositions()
{
    normalizedToCoords(currentKneeNormalized, currentWristNormalized, currentShoulderNormalized,
